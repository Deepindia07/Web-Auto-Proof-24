...
class LoginScreenBloc extends Bloc<LoginScreenEvent, LoginScreenState> {
  ...
  LoginScreenBloc({required this.authRepository, required this.userRole})
  ...
  Future<void> _handleEmployeeLogin(
    ...
    final dataBody = {'refCode': event.refNo};

    print("Employee login data: ${dataBody}");

    final resultResponse = await authRepository.loginEmployeeApiCall(
      dataBody: dataBody,
    );
    ...
    if (resultResponse.isSuccess) {
      ...
      SharedPrefsHelper.instance.setString(
        userId,
        "${resultResponse.data.inspector!.inspectorId}",
      );
      SharedPrefsHelper.instance.setString(
        emailKey,
        resultResponse.data.inspector!.email.toString(),
      );
      ...
      print(
        "Employee token: = ${SharedPrefsHelper.instance.getString(localToken)}",
      );
      emit(
        EmployeeLoginSuccess(employeeLoginResponseModel: resultResponse.data),
      );
    } else {
  ...
  Future<void> _handleRegularLogin(
    ...
    final resultResponse = await authRepository.loginApiCall(
      dataBody: dataBody,
    );
    ...
    if (resultResponse.isSuccess) {
      ...
      SharedPrefsHelper.instance.setString(
        userId,
        "${resultResponse.data.user!.userId}",
      );
      SharedPrefsHelper.instance.setString(
        emailKey,
        resultResponse.data.user!.email.toString(),
      );
      SharedPrefsHelper.instance.setString(isFirstTime, "isFirstTime");
      SharedPrefsHelper.instance.setString("userRole", userRole);

      print(
        "Regular token: = ${SharedPrefsHelper.instance.getString(localToken)}",
      );
  ...
  Future<void> _onEmailValidationCheck(
    ...
    final dataBody = {'email_or_phoneNumber': event.emailOrPhone};

    print("Validating email/phone: $dataBody");

    try {
      final resultResponse = await authRepository.forgotEmailCheckerApiCall(
        dataBody: dataBody,
      );
